...
- [Switchboard Params Receiver](#switchboard-params-receiver)
...
This SwitchboardParamsReceiver is a simple solidity contract where we pass some parameters to the callback function so we can handle individual orders in a batch.
...
async function main() {
  ...
  const calls = await Promise.all(paramsResults.map(async (paramsResult) => {
    ...
    // get txn
    return contract.populateTransaction.callback(bn);
  }));

  const contract = new Contract(
    "SWITCHBOARD_RECEIVER_ADDRESS",
    iface,
